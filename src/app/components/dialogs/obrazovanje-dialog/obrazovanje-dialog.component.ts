...
export class ObrazovanjeDialogComponent implements OnInit {
  ...
  public add(): void {
    this.obrazovanjeService.insertObrazovanje(this.data).subscribe(() => {
      this.snackBar.open('Uspešno dodato obrazovanje ' + this.data.naziv, 'OK', {duration:2500});
    }), (error:Error) => {
      this.snackBar.open('Došlo je do greške prilikom dodavanja novog obrazovanja',
  ...
  public update(): void {
    this.obrazovanjeService.updateObrazovanje(this.data).subscribe(() => {
      this.snackBar.open('Uspešno izmenjeno obrazovanje ' + this.data.naziv, 'OK', {duration:2500});
    }, (error:Error) => {
      this.snackBar.open('Došlo je do greške prilikom izmene obrazovanja',
      'Zatvori', {duration: 2500});
    });
  }
  ...
  public delete(): void {
    this.obrazovanjeService.deleteObrazovanje(this.data.id).subscribe(() => {
      ...
      this.snackBar.open('Došlo je do greške prilikom brisanja obrazovanja',
      'Zatvori', {duration: 2500});
    });
  }
